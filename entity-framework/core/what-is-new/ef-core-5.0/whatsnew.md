...
The collation to use for database columns can now be specified when building the model and will flow through to generated migrations.
For example:
...
Collations are also reverse-engineered from the database when scaffolding a DbContext.

The `EF.Functions.Collate()` allows for ad-hoc queries using different collations.
For example:
...
This can be useful for defining indexes, etc. over the column.
